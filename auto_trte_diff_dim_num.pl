...
foreach my $feature(@pick_feature){
	...
	for($start_num .. 10){
		...
		print "Dimension : ".$num."\n";
		print "----scale----\n";
		`svm-scale -s train-scale-info-sec result/combine_data/combine_e132_$feature-$num\_train > scale-train-data-sec`;
		`svm-scale -r train-scale-info-sec result/combine_data/combine_e132_$feature-$num\_test > scale-test-data-sec`;
		print "---complete--\n";
		print "----grid-----\n";
		`svm-grid -log2c -1,1,0.5 -log2g -10.5,-8.5,0.5 scale-train-data-sec`;
		print "---complete--\n";
		`sort -n -r -k 3 scale-train-data-sec.out > sort_grid-sec`;
		my $grid_str = `cat sort_grid-sec | head -n 1`;
		...
		print "----train-----\n";
		`svm-train -b 1 -h 0 -c $c_para -g $g_para scale-train-data-sec train-model-sec`;
		#`svm-train -b 1 -h 0 scale-train-data train-model`;
		print "----predict----\n";
		`svm-predict -b 1 scale-test-data-sec train-model-sec result/predict_report-new/predict_$feature-$num`;
